...
const getOrthoSnappedPoint = (startPoint, currentPoint) => {
 ...
 useEffect(() => { 
     ...
     console.log("OPERATION DATA LIGHTING: ",operationData)
...
if (!enabled() || !scene || !camera || !renderer) return;
    ...
    dispatch(setCommandMessage("Anahtar ekelemek için köşeleri seçin."));
   ...
   //     addTitleToScene(scene, position, wireNumber.toString(), '#ff00ff');
      }
...
if (verticesRef.current.length > 0 && previewPoint) {
  ...
  // Oluştur
  const symbolPreview = createLightSwich(
 ...
 const handleKeyDown = (e) => {
  ...
  const addLightSwitch=()=>{
    if (verticesRef.current.length > 1 && previewEndPointRef.current) { 
      ...
      lightSwichsRef.current.push({
        ...
        direction,
        power:0 })
      const lightSwitch = createLightSwich(scene, 
        p2.clone(),
  ...
  if (e.key === ' ') {
    addLightSwitch()
      ...
      dispatch(resetOperation());
      dispatch(setCommandMessage(''));
      console.log("snapSourceRef.current :  ",snapSourceRef.current)

      if(snapSourceRef.current?.type==="buatCircle"){
        const p1=snapSourceRef.current.position
        singleCircleBuatForLight(scene,p1, 0x00ffff,socket.userData.tempLineUUID)
        const tolerance = 0.1;
        let socket 
        scene.traverse((obj) => {
