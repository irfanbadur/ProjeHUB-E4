...
const useFindRoom = (scene, camera, renderer, snapPoints, options = {}) => {
    const {
        enabled = () => true,
        onPolylineComplete = () => { },
    } = options;
    ...
    useEffect(() => {
        setGlobaVerticesRef(verticesRef);
        setGlobalTempLineRef(tempLineRef);
      }, []);
    ...
    const createOffsetPolylinePreview = (polyline, offsetDistance = -5, scene, previewRef) => {
        ...
        if (isClosed) {
            points.pop();
            const scale = 1000; // float koordinatlar için ölçek
            const offsetDistance = -5;

            // Vector3[] → [ {X, Y} ] dizisine çevir
            const scaledInput = points.map(p => ({
                X: p.x * scale,
                Y: p.y * scale
            }));
            const co = new ClipperLib.ClipperOffset();
            // jtMiter = köşeler keskin (radius=0 gibi davranır)
            co.AddPath(scaledInput, ClipperLib.JoinType.jtMiter, ClipperLib.EndType.etClosedPolygon);
            /*
            etClosedPolygon	🔒 Kapalı poligon varsayar (default)
            etOpenButt	🔓 Açık çizgi, uçlar kesik (sade)
            etOpenSquare	🔳 Açık çizgi, uçlar kareleştirilir
            etOpenRound	🔵 Açık çizgi, uçlar yarım daire olur
            */
            const result = new ClipperLib.Paths();
            co.Execute(result, offsetDistance * scale);
            if (result.length === 0) return; // ❗ ÖNEMLİ: hiç offset çıkmamışsa çık
            const offsetVector3Array = result[0].map(p => new THREE.Vector3(p.X / scale, p.Y / scale, 0));
            offsetPoints = offsetVector3Array
        } else {
            // Açık polyline → uçlarda tek taraflı offset
            const scale = 1000; // float koordinatlar için ölçek
            const offsetDistance = -5;
        ...
        const previewLine = isClosed
            ...
            : new THREE.Line(geometry, material); // 👈 açık polyline için

        previewLine.computeLineDistances();
        previewLine.userData.isPreviewOffset = true;
        previewLine.userData.snapVertices = offsetPoints.map(p => p.clone()); // ✅ Buraya ekle
        previewLine.userData.id= generateUniqueId('polyline')
    ...
    useEffect(() => {
        if (commandType !== 'findRoom' || !scene || !camera || !renderer) return;
