...
import { createMainPanel } from '../../symbolDrawings/createMainPanel';

export const useMainPanel = (scene, camera, renderer) => {
  ...
  useEffect(() => {
    if (commandType !== 'drawMainPanel' || !scene || !camera || !renderer) return;
    ...
    const handleMouseMove = (event) => {
      ...
      if (previewPanelRef.current) {
        ...
        const panel = createMainPanel(scene, mousePos, "ADP");
...
const handleKeyDown = (e) => {
  if (e.key === 'Enter') {
    ...
    // ✅ 3) MainPanel oluştur
    const newPanel = createMainPanel(scene, mousePos, "ADP");
    scene.add(newPanel);
